...
class GoogleSheetsSync {
    ...
    loadConfig() {
        ...
        return {
            enabled: true,
            webhookUrl: '', // e.g. Google Apps Script Web App URL
     ...
     * Format matches image6.png:
     * [timestamp, User_ID, C_line, T_line, result, value, error]
     */
    async recordResult(record) {
        const rowData = {
            ...
            result: record.resultEnglish || (record.result === '양성' ? 'positive' : record.result === '음성' ? 'negative' : 'fail'),
            value: record.concentration !== null && record.concentration !== undefined ? record.concentration : '',
    ...
    exportCSV() {
        ...
        const headers = ['timestamp', 'User_ID', 'C_line', 'T_line', 'result', 'value', 'error'];
