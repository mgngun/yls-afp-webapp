...
class LFAAnalyzer {
    constructor(config = {}) {
        this.config = {
            ...
            autoChannelSelect: false,  // OFF by default: forces Green channel like the proven baseline.
                                        // Auto R/G/B selection is available as an opt-in (set true) but
                                        // was found to sometimes pick a channel with lower real contrast
                                        // on borderline strips — needs validation against real samples
    ...
    _selectActiveChannel(channelProfiles) {
        ...
        for (const name of names) {
            ...
            const { baseline, topHatProfile } = this._compensateIlluminationRobust(profile);
            const noiseSigma = this._computeNoiseSigma(topHatProfile);
    ...
    _detectPeaksRobust(rawProfile, correctedProfile, topHatProfile) {
        ...
        const bgNoiseSigma = this._computeNoiseSigma(matchedProfile);
    ...
    _validatePeak(detectSignal, quantSignal, startIdx, endIdx, absoluteMin, statisticalMin, bgNoiseSigma, lineType = 'T') {
        // Note: statisticalMin (matched-domain) and absoluteMin (raw-domain) are checked
        // separately below, each against the signal in the correct scale — see comments further down.
        let maxVal = 0;
        let bestIdx = -1;
        ...
        // Statistical significance gate — compared ONLY within the matched-filter (detect)
        // domain, since both peakHeightDetect and statisticalMin/minProm live in that same
        // (slightly attenuated-by-smoothing) scale. This must NOT be compared against
        // `absoluteMin`, which is a physical-absorbance constant calibrated against the RAW
        // top-hat scale — the Gaussian matched filter attenuates a line-width peak's amplitude
        // by roughly 20-30% (kernel spreads/dilutes it), so mixing the two scales here would
        // reject real borderline lines purely because of filter attenuation, not weak signal.
        const minProm = this.config.minLocalProminence || 0.004;
        if (peakIdx === -1 || peakHeightDetect < statisticalMin || bestProminence < minProm) {
            return {
                detected: false,
                ...
                zScore,
                rejectedReason: peakIdx === -1 ? 'no_local_maximum' : (bestProminence < minProm ? 'low_prominence' : 'below_statistical_threshold')
            };
        }

        // Snap to the true RAW local maximum near the matched-filter-localized peak.
        // The matched filter can shift the apparent peak center by a pixel or two versus
        // the true raw top-hat peak (especially on faint/borderline lines), so quantifying
        // height/AUC/FWHM at the raw signal's actual nearby maximum — not just ±1px around
        // the smoothed index — avoids under-reading real peak height.
        const snapRadius = Math.max(6, Math.round(this.config.matchedFilterSigma || 4));
        let qBestIdx = peakIdx;
        let qBestVal = quantSignal[peakIdx];
        for (let k = Math.max(startIdx, peakIdx - snapRadius); k <= Math.min(endIdx, peakIdx + snapRadius); k++) {
            if (quantSignal[k] > qBestVal) { qBestVal = quantSignal[k]; qBestIdx = k; }
        }
        peakIdx = qBestIdx;
        const peakHeight = qBestVal;

        // Absolute physical-absorbance floor — checked here, against the RAW (un-attenuated)
        // quantification height, which is the correct scale for a fixed real-world constant.
        if (peakHeight < absoluteMin) {
            return {
                detected: false,
                index: peakIdx,
                ...
                range: [startIdx, endIdx],
                zScore,
                rejectedReason: 'below_absolute_threshold'
            };
        }
